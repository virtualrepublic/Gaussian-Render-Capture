...
| Image Format | PNG | **PNG** or **TIFF** (LZW compression, smaller files), both RGBA 8 bit – set on every render; Postshot and LichtFeld Studio read both. If an earlier render in another format is still in the images folder, the export takes the chosen format. |
